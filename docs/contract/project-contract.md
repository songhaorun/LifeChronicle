...
和服务端 `main` 保护于 2026-08-11 激活。仓库采用长期单维护者模式，不以增加第二名
维护者为阶段完成条件；模式切换规则见第 18 节。
...
项目采用 bootstrap/单维护者模式，并可在没有第二名维护者时长期保持。仓库所有者
必须在验收记录中明确登记一名 bootstrap maintainer。该维护者可以：

- 将当前未提交分支改名为 `main`，提交规划文档和可逆仓库脚手架；
- 以版权持有者身份明确选择项目许可证，并提交 `LICENSE` 及所需通知文件；
- 对普通、可逆变更完成有记录的自审，前提是运行所有当前实际存在且适用的检查，
  并如实记录尚未实现的门禁；
- 在 CI 建立后通过受保护分支 PR、自审记录和成功检查推进非生产开发；单维护者策略
  不要求不存在的非作者或 CODEOWNERS 批准。
...
如果未来第二名维护者自愿加入，项目不会自动切换；只有角色登记、保护策略和版本化
治理记录明确宣布后，才进入多人治理模式，并完整启用第 13、15、16 节的非作者和多人
批准要求。具备独立评审能力后不得继续用单维护者条款绕过它。
...
1. 单维护者通过 PR 记录自审、实际检查和未满足项，并让必需 CI 成功；首次基线没有
   远程时可将等价记录纳入 commit；或
